...
__global__ void maxent_pool2d_forward_cuda_kernel(
    ...
    scalar_t* const top_data)
{
  ...
  if (ph < pooled_height) {
    ...
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        scalar_t val = bottom_slice[h * width + w] ;
        scalar_t expval = exp(lambda * val - maxval) ;
        numExp += val * expval ;
        denExp += expval ;
      }
    }
    ...
    // output
    top_data[oindex] = numExp / denExp ;
  }

}

template <typename scalar_t, bool OVERLAP>
__global__ void maxent_pool2d_backward_cuda_kernel(
  ...
  if (ph < pooled_height) {
    ...
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        ...
        numExp += val * expval ;
        denExp += expval ;
        numExp2+= val * val * expval ;
      }
    }
    
    // derivatives
    scalar_t datay = numExp / denExp ;
    scalar_t scl = dtop / denExp ;
    scalar_t* const bottom_diff_slice = bottom_diff + (n * channels + c) * height * width;
    if(OVERLAP){
      for (int h = hstart; h < hend; ++h) {
        for (int w = wstart; w < wend; ++w) {
          scalar_t val = bottom_slice[h * width + w] ;
          scalar_t grad= scl * exp(lambda * val - maxval) * (1 + lambda * (val - datay)) ;
    ...
    atomicAdd(param_diff + lm, dtop * (numExp2/denExp - datay*datay)) ;
  }
}
} // namespace
    ...
    bool ceil_mode)
{
  ...
  AT_CHECK(
    ...
    "maxent_pool2d: all IntArrayRef sizes must be 2");
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), 
    "maxent_pool2d_forward_cuda", 
    ([&] {
      scalar_t *output_data = output.data<scalar_t>();
      scalar_t *input_data = input.data<scalar_t>();
      scalar_t *param_data = param.data<scalar_t>();
      
      maxent_pool2d_forward_cuda_kernel<scalar_t>
      <<<blocks, num_threads>>>(
      input_data,
      param_data,
      nbatch,
  ...
  AT_CHECK( (status == cudaSuccess), "maxent_pool2d_forward_cuda failed with error code") ;
    ...
    bool ceil_mode)
{
  ...
  if(dH < kH || dW < kW ){ // overlapped pooling
    ...
    [&] {
      ...
      maxent_pool2d_backward_cuda_kernel<scalar_t, true>
      <<<blocks, num_threads>>>(
          gradOutput_data,
          input_data,
          param_data,
    ...
    [&] {
      ...
      maxent_pool2d_backward_cuda_kernel<scalar_t, false>
      <<<blocks, num_threads>>>(
          gradOutput_data,
          input_data,
          param_data,
  ...
  cudaError_t status = cudaGetLastError() ;
  AT_CHECK( (status == cudaSuccess), "maxent_pool2d_backward_cuda failed with error code") ;
}
